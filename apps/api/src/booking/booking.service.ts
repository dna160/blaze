...
export class BookingService {
  ...
  private async createStorageBooking(
    ...
    customer: NotifiableCustomer & { isBlocklisted: boolean },
    ...
    const { booking, assetCode, waitlistPosition } = await this.prisma.runInTenantContext(tenant.id, async (tx) => {
      ...
      if (!assetId) {
        ...
        const position = await tx.booking.count({
          where: { status: "WAITLISTED", assetTypeId: assetType.id, locationId, waitlistedAt: { lte: created.waitlistedAt! } },
        });
   ...
   * branch, first invoice, contract). Waitlist position is per branch +
   * size, oldest first.
   */
    ...
    return this.prisma.runInTenantContext(tenant.id, async (tx) => {
      ...
      return bookings.map((b) => {
        ...
        if (b.status === "WAITLISTED") {
          const key = `${b.locationId ?? ""}:${b.assetTypeId}`;
          waitlistPosition = (waitlistCounters.get(key) ?? 0) + 1;
          waitlistCounters.set(key, waitlistPosition);
        }
        return {
          id: b.id,
          status: b.status,
