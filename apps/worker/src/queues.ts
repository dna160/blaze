...
/**
 * Both jobs run daily (PRD §8.2: "Recurring generator runs daily"; §7.2.4
 * dunning ladder). Registered as BullMQ repeatable jobs rather than a
 * plain setInterval so retries, concurrency, and observability come from
 * the same queue infra as everything else in the stack (PRD §11: "BullMQ
...
export function startWorkers(connection: IORedis): Worker[] {
  ...
  for (const worker of [invoiceWorker, dunningWorker]) {
    worker.on("failed", (job, err) => console.error(`Job ${job?.id} in ${job?.queueName} failed:`, err));
  }

  return [invoiceWorker, dunningWorker];
}
