...
**Session 28 shipped PRD v2** (`docs/PRD-v2-storage-flow.md`) on branch
`claude/rentos-v2-storage-flow` — not merged into the Railway-deployed
branch, because it carries a migration that must run first. Start by
reading that PRD's §2/§3 decision tables and Session 28's entry above;
the natural next items, in the owner's own priority order, are the
per-tenant WhatsApp Business API setup screen and Xendit live keys, then
the portal "Extend" button (backend already supports it) and automatic
waitlist promotion. `scripts/smoke-v2.sh` is the fastest way to confirm
the whole storage flow still works after any change.
